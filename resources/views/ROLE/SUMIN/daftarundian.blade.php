                            ...
                            <div class="card">
                                <div class="card-header border-transparent">
                                    ...
                                    <div class="card-tools">
                                        <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalTambahUndian">
        ...
        @foreach($dftrundian as $undian)
            <tr>
                ...
                <td>{{ $undian->tanggal_kocok }}</td>
                    <td>{{ $undian->pemenang ?? 'Belum Ada' }} - {{ $pelanggan[$undian->pemenang]->akun_pppoe ?? 'Tidak Ada' }}

                    </td>
                <td>
                    @if($undian->foto_pemenang)
                        <img src="{{ asset('/undian/pemenang/' . $undian->foto_pemenang) }}" alt="Foto Pemenang" class="img-thumbnail" style="width: 100px; height: 100px;">
                    @else
                        <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#uploadModal" data-id="{{ $undian->id }}">Upload Foto</button>
                    @endif
                </td>
                <td>
                    ...
                    <a href="" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadModalLabel">Upload Foto Pemenang</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('upload.foto.pemenang') }}" method="POST" enctype="multipart/form-data">
            @csrf
                <div class="modal-body">
                    <input type="hidden" name="undian_id" id="undian_id">
                    <div class="form-group">
                        <label for="foto_pemenang">Pilih Foto</label>
                        <input type="file" class="form-control-file" name="foto_pemenang" required>
                    </div>
                </div>
        ...
        <!-- Modal Tambah Undian -->
<div class="modal fade" id="modalTambahUndian" tabindex="-1" role="dialog" aria-labelledby="modalTambahUndianLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <form action="{{route('undian.tambahundian')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="nama_undian" placeholder="Nama Undian" required>
                    </div>
